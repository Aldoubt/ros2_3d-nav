...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node)
{
  ...
  try_declare.template operator()<std::string>("frames.world_frame", "camera_init");
  try_declare.template operator()<std::string>("frames.body_frame", "aft_mapped");
  ...
  this->node->get_parameter("frames.world_frame", world_frame);
  this->node->get_parameter("frames.body_frame", body_frame);
