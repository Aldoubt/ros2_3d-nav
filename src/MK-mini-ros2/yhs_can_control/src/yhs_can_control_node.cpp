...
namespace yhs
{
	...
	CanControl::CanControl(rclcpp::Node::SharedPtr node)
			: node_(node), if_name_("can0"), odom_topic_("wheel_odom"), can_socket_(-1), wheel_base_(0.6), publish_wheel_odom_(true)
	{

		READ_PARAM(std::string, "can_name", (if_name_), "can0");
		READ_PARAM(double, "wheel_base", (wheel_base_), 0.6);
		READ_PARAM(std::string, "odom_topic", (odom_topic_), "wheel_odom");
		READ_PARAM(bool, "publish_wheel_odom", (publish_wheel_odom_), true);
		...
		if (publish_wheel_odom_)
		{
			odom_pub_ = node_->create_publisher<nav_msgs::msg::Odometry>(odom_topic_, 1);
		}
	}
...
whether the can line is connected correctly, and whether the chassis is powered on.");
	...
	{
		if (!publish_wheel_odom_ || !odom_pub_)
		{
			return;
		}
