...
free_thresh: 0.196
  ...
  bool Load(const std::string &yaml_path) {
    ...
    QImage img(QString::fromStdString(map_config.image));
    if (img.isNull()) {
      LOG_ERROR("failed to open image file \"" << map_config.image << "\"");
      return false;
    }
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        const QColor pixel_color = img.pixelColor(x, y);

        std::vector<unsigned char> channels{
          static_cast<unsigned char>(pixel_color.red()),
          static_cast<unsigned char>(pixel_color.green()),
          static_cast<unsigned char>(pixel_color.blue())
        };

        const bool alpha_channel_exists = pixel_color.alpha() < 255;
        const unsigned char alpha_value = static_cast<unsigned char>(pixel_color.alpha());
