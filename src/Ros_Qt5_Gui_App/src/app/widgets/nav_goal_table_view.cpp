...
void NavGoalTableView::UpdateTopologyMap(const TopologyMap &_topology_map) {
  ...
  for (const auto &point : topologyMap_.points) {
    point_names << QString::fromStdString(point.name);
  }
  for (int row = 0; row < table_model_->rowCount(); ++row) {
    ...
    const QString current_text = comboBox->currentText();
    comboBox->blockSignals(true);
    comboBox->clear();
    comboBox->addItems(point_names);
    comboBox->addItem("");
    const int current_index = comboBox->findText(current_text);
    if (current_index >= 0) {
...
bool NavGoalTableView::LoadTaskChain(const std::string &name) {
  ...
  for (auto point : task_chain_.points) {
    ...
    connect(button_run, &QPushButton::clicked, [this, comboBox, label_status]() {
      TopologyMap::PointInfo selected_point =
          topologyMap_.GetPoint(comboBox->currentText().toStdString());
      if (selected_point.name.empty()) {
        label_status->setText("Point Not Found!");
        return;
      }
      label_status->setText("Running");
      emit signalSendNavGoal(selected_point.ToRobotPose());
    });
    table_model_->insertRow(row);
