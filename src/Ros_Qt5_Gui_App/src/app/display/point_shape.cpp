...
PointShape::PointShape(const ePointType &type, const std::string &display_type,
  ...
  switch (type_) {
    case kRobot: {
      setZValue(10);
      point_pixmap_.load(QString("://images/robot.svg"));
      deg_offset_ = 45;
      SetBoundingRect(QRectF(
          0 - point_pixmap_.width() / 2,
          0 - point_pixmap_.height() / 2,
          point_pixmap_.width(),
          point_pixmap_.height()));
    } break;
    case kParticle: {
    } break;
    case kNavGoal: {
      setZValue(9);
      point_pixmap_.load(QString("://images/target.svg"));
      deg_offset_ = 0;
      SetBoundingRect(QRectF(
          0 - point_pixmap_.width() / 2,
          0 - point_pixmap_.height() / 2,
...
void PointShape::drawRobot(QPainter *painter) {
  ...
  QRectF targetRect(
      -point_pixmap_.width() / 2,
      -point_pixmap_.height() / 2,
      point_pixmap_.width(),
      point_pixmap_.height());
  painter->drawPixmap(targetRect.toRect(), point_pixmap_);
  painter->restore();
}
void PointShape::drawNavGoal(QPainter *painter) {
  ...
  // 绘制不旋转的目标SVG图标
  QRectF targetRect(-point_pixmap_.width() / 2,
                    -point_pixmap_.height() / 2,
                    point_pixmap_.width(),
                    point_pixmap_.height());
  painter->drawPixmap(targetRect.toRect(), point_pixmap_);
